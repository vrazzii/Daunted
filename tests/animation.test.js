...
test("every idle frame is locked to the same ground origin", () => {
  for (let frame = 0; frame < idle.frameCount; frame += 1) {
    assert.deepEqual(frameRenderOffset(idle, frame), { x: 0, y: 0 });
    assert.deepEqual(idle.frameAnchors[frame], idle.origin);
  }
});

test("per-frame anchor corrections eliminate art drift", () => {
  const corrected = {
    ...idle,
    frameAnchors: idle.frameAnchors.map((anchor, index) =>
      index === 1 ? { x: anchor.x + 3, y: anchor.y - 2 } : anchor
    )
  };
