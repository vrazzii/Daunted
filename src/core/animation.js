...
export function validateAnimationDefinition(animation) {
  ...
  if (!Array.isArray(animation.frameAnchors)) {
    errors.push("frameAnchors must be an array");
  } else if (animation.frameAnchors.length !== animation.frameCount) {
    errors.push("frameAnchors length must equal frameCount");
  } else {
    animation.frameAnchors.forEach((anchor, index) => {
      if (
        !anchor ||
        !Number.isFinite(anchor.x) ||
        !Number.isFinite(anchor.y) ||
        anchor.x < 0 ||
        anchor.x > animation.frameWidth ||
        anchor.y < 0 ||
        anchor.y > animation.frameHeight
      ) {
        errors.push(`frameAnchors[${index}] must be inside the frame cell`);
      }
    });
  }
...
export function frameRenderOffset(animation, logicalFrameIndex) {
  const normalized = normalizeFrame(logicalFrameIndex, animation.frameCount);
  const anchor = animation.frameAnchors[normalized];

  return {
    x: animation.origin.x - anchor.x,
    y: animation.origin.y - anchor.y
  };
}
